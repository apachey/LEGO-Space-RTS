...
4. **Automated verification** — `./tools/verify.sh` is the fast gate. Determinism, navigation, movement, save/replay, milestone, or large changes also require `./tools/verify.sh --full`. A failing gate blocks completion; skipped checks are reported as `SKIPPED` with a reason.
