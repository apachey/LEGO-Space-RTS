...
**M2 — complete and accepted. M3 is the next approved milestone.**
...
non-blocking polish rather than further M2 work. M3 has not begun.
...
1. Bootstrap M3 from its directly relevant canon and current merged repository
   state.
2. Identify and implement the smallest approved M3 task.
