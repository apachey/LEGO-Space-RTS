...
- Rejected V1 blind-review code: `S14`. Historical failed boards: [24-cell](../Silhouettes/blind_24_cells.svg), [44-cell](../Silhouettes/blind_44_cells.svg), [72-cell](../Silhouettes/blind_72_cells.svg). The game director recognized 0/66 at 24 cells; this primitive concept is not an approved model or accepted evidence.
...
- Semantic part map:
  - One huge interrupted circular carrier hull — defines the single selectable flagship — SOURCE_VERIFIED/CANON_DERIVED_ADAPTATION.
  - Open central lime-conduit channel and long directional tail — prevent a generic sealed saucer — SOURCE_VERIFIED.
  - Integrated front, side and jetpack bays — unfold to expose carried craft and support functions without splitting the Mothership entity — SOURCE_VERIFIED/CANON_DERIVED_ADAPTATION.
- Structural load path: A flattened circular frame carries the open machinery channel, directional tail and several bay interfaces; every opening section remains connected to the main carrier root.
- Repeated modules / connection grammar: Main carrier ring, one long central/front crew craft, two mirrored seated side craft, two outer single-figure jetpack bays, tail blades and Charge-relay equipment form one unit; source crew stations remain exposed.
- Source-faithful versus adapted boundary: The completed source assembly shows all five docked subcraft/jetpack positions together. Those source placements do not define five gameplay passengers, buildable units, launch timing or internal production. Source modules are analyzed separately but may not become separate Mothership entities. Training units inside is a proposal, not a locked T082 mechanic.

## E. Material and texture contract

- Geometry must carry:
  - interrupted circular carrier hull
  - open central channel and long tail
  - integrated unfolding craft bays
...
- Locomotion / operation: Huge true-air carrier with deliberate slow banking.
- Planted/contact rule: No battlefield ground contact; launch/support presentation uses stable airborne loiter.

| Pivot | Parent | Axis/path and rest-to-extreme motion | Presentation driver |
|---|---|---|---|
| `Pivot_CentralBay` | Asset_AlienMothership | closed travel relationship to open internal-channel access | load, unload or reinforcement presentation |
| `Pivot_SideBayLeft` | Asset_AlienMothership | connected outward unfolding arc | carrier/support presentation |
| `Pivot_SideBayRight` | Asset_AlienMothership | mirrored connected unfolding arc | carrier/support presentation |

- Required beats:
  - Idle carrier loiter with bays closed enough to preserve silhouette.
  - Travel keeps all sections visibly integrated.
  - Load/unload or reinforcement opens only the required connected bay.
  - Charge support brightens the conduit path; destruction breaks outer modules after carrier failure.
- Animation consumes authoritative state and never decides gameplay timing or results.

## G. Presentation hookups

- Required presentation sockets: `Socket_Selection`, `Socket_Health`, `Socket_Cargo`, `Socket_Load`, `Socket_Unload`, `Socket_SurgeRelay`, `Socket_BayLeft`, `Socket_BayRight`, `Socket_AudioFlight`.
...
- Remaining source/design decisions:
  - Proposed internal unit production still needs an explicit decision on eligible units, cost, time, capacity interaction and whether an unfolded state is required.
