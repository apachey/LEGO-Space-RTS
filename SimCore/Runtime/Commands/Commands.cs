...
{
public enum SimCommandType : ushort { Move = 1, Stop = 2, HoldPosition = 3, Harvest = 4, Build = 5, CancelConstruction = 6, AssistConstruction = 7, QueueProduction = 8, SetRallyPoint = 9, SetEnergyPriority = 10, MissionRefit = 11, SetResonanceCommitment = 12, StartSurge = 13, DebugOpenExcavatable = 1000, DebugDrainEnergy = 1001 }
...
public readonly struct CommandEnvelope
{
    ...
    {
        if (type != SimCommandType.Move && type != SimCommandType.Stop && type != SimCommandType.HoldPosition && type != SimCommandType.Harvest && type != SimCommandType.Build && type != SimCommandType.CancelConstruction && type != SimCommandType.AssistConstruction && type != SimCommandType.QueueProduction && type != SimCommandType.SetRallyPoint && type != SimCommandType.SetEnergyPriority && type != SimCommandType.MissionRefit && type != SimCommandType.SetResonanceCommitment && type != SimCommandType.StartSurge && type != SimCommandType.DebugOpenExcavatable && type != SimCommandType.DebugDrainEnergy)
