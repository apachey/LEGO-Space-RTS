...
public static class AuthoritativeStateDumper
{
    public static string Dump(SimulationWorld world)
    {
        ...
        for(int i=0;i<alive.Count;i++)
        {
            ...
            if(world.Entities.Targeting.TryGet(id,out Targeting targeting))b.Append(" currentTarget=").Append(targeting.CurrentTarget.Value).Append(" targetKind=").Append((byte)targeting.SelectionKind).Append(" targetPriority=").Append((byte)targeting.PriorityProfile).Append(" acquireRadius=").Append(targeting.AcquisitionRadius.Raw);
