...
if [[ "${CAPTURE_MODE}" == "--construction" ]]; then
  ...
  "${GODOT}" --log-file "${CAPTURE_LOG}" --quit-after 600 --path "${ROOT}/GodotClient" -- --capture-smoke --capture-path "${OUTPUT}" --capture-repair
elif [[ "${CAPTURE_MODE}" == "--transport" ]]; then
  "${GODOT}" --log-file "${CAPTURE_LOG}" --quit-after 600 --path "${ROOT}/GodotClient" -- --capture-smoke --capture-path "${OUTPUT}" --capture-transport
elif [[ "${CAPTURE_MODE}" == "--transformation" ]]; then
  "${GODOT}" --log-file "${CAPTURE_LOG}" --quit-after 600 --path "${ROOT}/GodotClient" -- --capture-smoke --capture-path "${OUTPUT}" --capture-transformation
elif [[ "${CAPTURE_MODE}" == "--transformation-rollback" ]]; then
  "${GODOT}" --log-file "${CAPTURE_LOG}" --quit-after 600 --path "${ROOT}/GodotClient" -- --capture-smoke --capture-path "${OUTPUT}" --capture-transformation-rollback
