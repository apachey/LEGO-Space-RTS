...
CAPTURE_MODE="${2:-}"
if [[ -n "${CAPTURE_MODE}" && "${CAPTURE_MODE}" != "--construction" && "${CAPTURE_MODE}" != "--repair" && "${CAPTURE_MODE}" != "--transport" && "${CAPTURE_MODE}" != "--transformation" && "${CAPTURE_MODE}" != "--transformation-rollback" ]]; then printf 'Usage: %s [output.png] [--construction|--repair|--transport|--transformation|--transformation-rollback]\n' "$0" >&2; exit 2; fi
...
if [[ "${CAPTURE_MODE}" == "--construction" ]]; then
  ...
  "${GODOT}" --quit-after 600 --path "${ROOT}/GodotClient" -- --capture-smoke --capture-path "${OUTPUT}" --capture-transformation
elif [[ "${CAPTURE_MODE}" == "--transformation-rollback" ]]; then
  "${GODOT}" --quit-after 600 --path "${ROOT}/GodotClient" -- --capture-smoke --capture-path "${OUTPUT}" --capture-transformation-rollback
else
