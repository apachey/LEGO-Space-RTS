...
CAPTURE_MODE="${2:-}"
if [[ -n "${CAPTURE_MODE}" && "${CAPTURE_MODE}" != "--construction" && "${CAPTURE_MODE}" != "--repair" && "${CAPTURE_MODE}" != "--transport" && "${CAPTURE_MODE}" != "--transformation" ]]; then printf 'Usage: %s [output.png] [--construction|--repair|--transport|--transformation]\n' "$0" >&2; exit 2; fi
