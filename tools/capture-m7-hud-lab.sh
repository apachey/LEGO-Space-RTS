...
usage() {
  printf 'Usage: %s [output.png] [rock-unit|mixed-army|production|brownout|astronaut-transform|alien-resonance|martian-network|critical-tooltip] [16-9|16-10|21-9|4-3] [visible|hidden] [hybrid|structural|legacy|clean] [faction|light|sandstone|oxide|olive|alien|graphite|custom]\n' "$0" >&2
}
...
if [[ ! -s "${OUTPUT}" ]] || ! grep -q "M7 HUD LAB: PASS scenarios=8 commands=12 minimap=legal.*factionSkins=4.*schema=8 active=${SCENARIO} finish=${EXPECTED_FINISH} palette=${EXPECTED_PALETTE}" "${CAPTURE_LOG}"; then
