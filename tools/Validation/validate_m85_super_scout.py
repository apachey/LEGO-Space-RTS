...
def validate_mt101_source_rebuild() -> None:
    ...
    for item in (record, review):
        if (item.get("stableId"), item.get("status"), item.get("productionAccepted"), item.get("canonImpact")) != (
                "unit.astronauts.mt101_armored_drilling_unit", "DIRECTOR_ACCEPTED_APPEARANCE_FOR_COMPARATIVE_REVIEW", False, "NONE"):
            fail("MT source rebuild expanded appearance acceptance beyond comparative review")
    ...
    approval = {
        "baseCommit": "d447a2c",
        "message": "так, це воно, готово",
        "scope": "Rev5 MT-101 completed appearance only; comparative review acceptance, not production integration, gameplay, or exact geometry.",
    }
    if record.get("approvalEvidence") != approval or review.get("approvalEvidence") != approval:
        fail("MT source rebuild lost exact director approval evidence or comparative-only scope")
    if record.get("history") != {"freeFormAttempts": 2, "nativeConstructionApproaches": 1, "controlledRasterFinishes": 1, "targetedNestedCorrections": 2, "sourceRebuildGenerations": 5}:
    ...
    expected_attempts = [
        ...
        {"number": 4, "file": "mt101_source_rebuild_rev4.png", "sha256": "aecb0822dbc854e319786ab115c30e6355a38ef7f955ed9ff987e27106d8675d", "prompt": "prompt_rev4.txt", "promptSha256": "c2022190b12d6daa744f88d52b39bd03d26e9b601d65ec3adbd3ffc8948a08c5", "finding": "REJECTED_MISSING_FULL_ZAMOR_LAUNCHER"},
        {"number": 5, "file": "mt101_source_rebuild_rev5.png", "sha256": "4e6583ce4eb2ae7941967a23a2692358a42abbfc191b831a99c1dac2c1d1752c", "prompt": "prompt_rev5.txt", "promptSha256": "b25d818aa3036b43880c0625c4c2399da3d22b14d716d9e219981078eb5a9280", "finding": "DIRECTOR_ACCEPTED_APPEARANCE_FOR_COMPARATIVE_REVIEW"}]
    attempts = record.get("attempts", [])
    attempt_core = [
    ...
    expected_input_chains = [
        ...
        [
            ("ArtSource/M85/Preproduction/MT101SourceRebuildV1/mt101_source_rebuild_rev4.png", "aecb0822dbc854e319786ab115c30e6355a38ef7f955ed9ff987e27106d8675d", "PREVIOUS_SOURCE_REBUILD_EDIT_CANVAS_REJECTED_FOR_MISSING_FULL_ZAMOR_LAUNCHER"),
            ("ArtSource/M85/Preproduction/MT101SourceRebuildV1/reference_book2_p35.png", "9c6ac3ec875ba64c717d78becf0825df41f65c640f32b213555910bd76333f54", "OFFICIAL_LAUNCHER_CONSTRUCTION_SCALE_AND_PLACEMENT_CONTROL"),
            ("ArtSource/M85/Preproduction/MT101ControlledAppearanceV1/reference_7699_book2_cover.png", "cbb37fddf08f16da2300178cfe97ecf72d91c9caff56be04fb3186c81f69fcdd", "OFFICIAL_ASSEMBLED_LAUNCHER_SILHOUETTE_CONTROL"),
        ],
    ]
    actual_input_chains = [
    ...
    if record.get("inputPolicy") != "Rev1 uses five official references. Rev2-rev5 use the previous source-rebuild raster only as an edit canvas plus official correction controls. Rejected pre-rebuild generated and native shapes are excluded.":
        fail("MT source rebuild obscured edit-canvas provenance")
    if record.get("inspection") != {"closedSteepCabin": True, "curvedStuddedShells": True, "broadBarrelWheels": True, "twoStarDrillCutters": True, "rearDrillCupCollar": True, "fullCrossbowZamorLauncherVisible": True, "launcherAboveCabin": True, "launcherSeparateFromDrill": True, "launcherScaleSourceControlled": True, "rearCraftAttached": True, "rearCraftWedgeCanopy": True, "twoFarRearFins": True, "visibleLowWingPlates": 1, "secondLowWingOccluded": True, "twoLowWingRasterTopologyProven": False, "visibleChassisWheels": 4, "sixWheelRasterTopologyProven": False, "bikeStowageProvenByRaster": False, "exactSourceGeometryProven": False, "gameplayCameraAccepted": False}:
    ...
    if review.get("pending") != ["Final gameplay-camera and production review", "Separate director gameplay decision for independent rear spacecraft and mini-bike operation"]:
        fail("MT source rebuild hid production or gameplay gates")
    finding = review.get("finding", "")
    if ("full" not in finding.lower() or "Zamor" not in finding or "above" not in finding.lower()
            or "One low outer wing plate is visible" not in finding or "occluded plate" not in finding):
        fail("MT source rebuild lost full launcher finding or overstated hidden low-wing proof")
    historical_boundary = review.get("historicalBoundary", "").lower()
    if "rev4" not in historical_boundary or not any(term in historical_boundary for term in ("missing", "lacks")):
        fail("MT source rebuild hid rejected Rev4 launcher failure")
    ...
    if "separate prominent horizontal Bionicle Zamor sphere launcher above the cabin" not in modules or "fuse with the Zamor launcher" not in boundary or "secondary gun/tool" in modules + boundary or "compact Bionicle Zamor" in modules + boundary:
        fail("MT source rebuild lost exact Zamor launcher terminology")
    research = record.get("research", {})
    if research != {"file": "Docs/Development/M85_T082_MT101_SOURCE_REBUILD_RESEARCH.md", "sha256": "689d6dc2f05ca184adde5a74fedb004ba6b737d000e2d739e125baa4958094b2"} or hashlib.sha256((ROOT / research.get("file", "missing")).read_bytes()).hexdigest() != research.get("sha256"):
...
def validate_current_blind_review() -> None:
    ...
    expected_identity = (
        ...
        "BLOCKING_NOW",
        "HOLD_FOR_GAME_DIRECTOR_44_CELL_REVIEW",
        False,
        "NONE",
    )
    actual_identity = (
    ...
    if (
        ...
        or record.get("nextReviewWidthCells") != 44
...
def main() -> None:
    ...
    if blind_review_results.get("status") != "CURRENT_BLIND_REVIEW_V1_AWAITING_GAME_DIRECTOR_44_CELL":
        fail("blind-review result must preserve the renewed 44-cell director-review gate")
    ...
    expected_renewed_pages = [
        "Docs/Development/M85SuperScout/Silhouettes/FullV2/CurrentBlindReviewV1/blind_44_cells_page_1.png",
        "Docs/Development/M85SuperScout/Silhouettes/FullV2/CurrentBlindReviewV1/blind_44_cells_page_2.png",
    ]
    if (
        active_renewed.get("id") != "T082_CURRENT_SELECTED_RENEWED_V1"
        or active_renewed.get("state") != "AWAITING_GAME_DIRECTOR_44_CELL_REVIEW"
        or active_renewed.get("preparedCameraWidthsCells") != [24, 44, 72]
        or active_renewed.get("activeCameraWidthCells") != 44
        ...
        or active_renewed.get("director24CellFeedback") != {
            "verbatim": "йдемо далі, наче все зрозуміло",
        ...
        fail("renewed current review must preserve unscored 24-cell feedback and await 44-cell director evidence")
