...
if [[ ! -s "${OUTPUT}" ]] || ! grep -q "M7 LOOK LAB: PASS schema=3 units=4 meshes=192 triangles=31104 buildings=2 firing=1 burning=1 animationDrivers=4 vfxPools=3 prewarmed=144 controls=${CONTROLS} zoom=${ZOOM} post=${POST} outline=${OUTLINE}" "${CAPTURE_LOG}"; then
