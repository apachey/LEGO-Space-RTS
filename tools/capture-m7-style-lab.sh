...
OUTLINE="${3:-off}"
CAPTURE_LOG="${TMPDIR:-/tmp}/lego-space-rts-m7-style-${STYLE}-${OUTLINE}.log"

case "${STYLE}" in
  industrial-mass|heroic-rts|constructive-lego|graphic-volume) ;;
  *) printf 'Usage: %s [industrial-mass|heroic-rts|constructive-lego|graphic-volume] [output.png] [off|on]\n' "$0" >&2; exit 2 ;;
esac
case "${OUTLINE}" in
  off|on) ;;
  *) printf 'Usage: %s [industrial-mass|heroic-rts|constructive-lego|graphic-volume] [output.png] [off|on]\n' "$0" >&2; exit 2 ;;
