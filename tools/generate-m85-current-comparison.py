...
def selections() -> list[dict]:
    ...
    # Completed nested correction supersedes the old incomplete view, not approval.
    mt = read(ROOT / "Content/Presentation/SuperScout/mt101_nested_appearance_review.json")
    selected[mt["stableId"]] = {k: mt[k] for k in ("stableId", "file", "sha256", "status")}
    selected[mt["stableId"]]["origin"] = "mt101_nested_appearance_review.json"
...
<p>Це огляд із назвами, не сліпий тест і не перевірка масштабу в грі. Старі базові картинки залишені лише як контекст, не як автоматично погоджений вигляд. MT-101 має нову завершену пропозицію: зібрана машина та задній зореліт із вийнятим двоколісним мінібайком. Вигляд ще не погоджений; картинка не доводить точність конструкції чи роботу в грі. У Mothership досі не погоджені оператори та рельєф замість друку.</p>
