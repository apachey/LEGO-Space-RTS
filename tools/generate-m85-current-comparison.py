...
def selections() -> list[dict]:
    ...
    # Source rebuild supersedes director-rejected nested appearance, not approval.
...
<p>Це огляд із назвами, не сліпий тест і не перевірка масштабу в грі. Старі базові картинки залишені лише як контекст, не як автоматично погоджений вигляд. Попередній MT-101 відхилений; нова завершена пропозиція відтворює оригінальний набір за офіційними зображеннями. Зореліт приєднаний позаду, мінібайк прихований усередині. Вигляд ще не погоджений; картинка не доводить точність конструкції чи роботу в грі. У Mothership досі не погоджені оператори та рельєф замість друку.</p>
